...
- Context-aware failure recovery links in notifications and retained task output
...
Pick a project from the switcher, or use **Create New Project** to scaffold one,
then work through the dashboard's development, build, update, submit, and
...
[issue tracker](https://github.com/karodgers/expo-workflows/issues) rather than
filing a public report with exploit details.
...
2. Keep changes to the webview (`src/webview/`) and the extension host (`src/`)
   isolated to their message contract in `src/webviewProtocol.ts`.
